...
function okData(result) { return result?.ok ? result.data : null; }
function asArray(value) { return Array.isArray(value) ? value : []; }
function normalisePlanetName(value) { return String(value || '').trim().toLowerCase(); }
function findPlanet(rows, wanted) {
  const target = normalisePlanetName(wanted);
  return asArray(rows).find(row => normalisePlanetName(row.planet_name || row.name) === target) || null;
}
...
function selectedPeriodFromAll(allData, level, reportDate) {
  return asArray(allData?.[level]?.dasha_period).find(period => periodContains(period, reportDate)) || null;
}
...
function compareCurrentDasha(currentData, allData, reportDate) {
  ...
  REQUIRED_DASHA_LEVELS.forEach(level => {
    ...
    const agrees = Boolean(directPlanet && selectedPlanet && normalisePlanetName(directPlanet) === normalisePlanetName(selectedPlanet));
    comparisons[level] = {
...
function compareNumerology(deterministic, bundle) {
  ...
  const apiValues = {
    psychic_number: { value: findNestedNumber(indian, ['radical_number', 'radical_num', 'psychic_number', 'driver_number']), method: 'Indian radical number' },
    destiny_number: { value: findNestedNumber(indian, ['destiny_number', 'conductor_number']) ?? findNestedNumber(western, ['lifepath_number', 'life_path_number']), method: 'Digit-sum destiny/life-path number' },
    name_number: { value: findNestedNumber(western, ['expression_number']), method: 'Pythagorean expression number' }
  };
  const comparison = {};
  const discrepancies = [];
  ['psychic_number', 'destiny_number', 'name_number'].forEach(key => {
    ...
    comparison[key] = { backend, astrologyapi: api, astrologyapi_method: apiValues[key].method, comparable, agrees };
    if (comparable && !agrees) discrepancies.push(`NUMEROLOGY VERIFICATION REQUIRED: backend ${key} is ${backend}, while the compatible AstrologyAPI ${apiValues[key].method} returns ${api}.`);
  });
...
function verifySourceBundle(bundle, deterministicNumerology, { reportDate = new Date() } = {}) {
  ...
  Object.entries(requiredSources).forEach(([name, available]) => { if (!available) blockingIssues.push(`DATA REQUIRED: ${name} is unavailable.`); });
  ...
  if (Array.isArray(cusps) && cusps.length !== 12) blockingIssues.push(`DATA REQUIRED: expected 12 KP house cusps, received ${cusps.length}.`);
  if (Array.isArray(planetSignificators) && planetSignificators.length < 9) warnings.push(`KP planet significators returned ${planetSignificators.length} rows; expected at least 9.`);
  if (Array.isArray(houseSignificators) && houseSignificators.length !== 12) warnings.push(`KP house significators returned ${houseSignificators.length} rows; expected 12.`);
  ...
  if (regularAscDegree !== null && kpAscDegree !== null && Math.abs(regularAscDegree - kpAscDegree) > 0.05) {
    blockingIssues.push(`CHART VERIFICATION REQUIRED: regular and KP Ascendant degrees differ by ${Math.abs(regularAscDegree - kpAscDegree).toFixed(4)}°.`);
  }
  ...
  const dashaVerification = currentDasha && allDasha ? compareCurrentDasha(currentDasha, allDasha, date) : { comparisons: {}, discrepancies: [] };
...
module.exports = { compareCurrentDasha, compareNumerology, verifySourceBundle };