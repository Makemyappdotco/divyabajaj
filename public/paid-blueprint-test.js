(function(){
  ...
  var lastFreePayload = null;

  function qs(sel, root){ return (root || document).querySelector(sel); }
  function qsa(sel, root){ return Array.prototype.slice.call((root || document).querySelectorAll(sel)); }
  function txt(el){ return (el && el.textContent || '').trim().toLowerCase(); }

  function getValue(id){ var el = document.getElementById(id); return el ? String(el.value || '').trim() : ''; }
    ...
    modal.innerHTML = '\
      <div class="pb-card" role="dialog" aria-modal="true" aria-labelledby="pbTitle">\
        ...
        <div class="pb-head">\
          ...
          <p class="pb-sub">Payment gateway is skipped for testing. This form generates the full paid report using the paid model.</p>\
        </div>\
        <div class="pb-body">\
  ...
  function openPaidBlueprint(){
    ...
    var known = Object.assign({}, readKnownFormValues(), lastFreePayload || {});
  ...
  function closePaidBlueprint(){
    var modal = document.getElementById('paidBlueprintModal');
  ...
  async function submitPaidBlueprint(e){
    ...
    if (missing.length) {
      err.textContent = 'Please fill: ' + missing.join(', ');
