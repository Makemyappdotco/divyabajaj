const SCORING_VERSION = 'dominant-planets-v1-draft';
...
function findKpPlanet(rows, planet) {
  ...
  return (Array.isArray(rows) ? rows : []).find(row =>
    normalisePlanet(row?.planet_name || row?.name) === target
  ) || null;
}
...
function addDashaRoles(map, currentDasha) {
  Object.entries(DASHA_WEIGHTS).forEach(([level, weight]) => {
    const period = currentDasha?.[level];
...
function scoreDominantPlanets(bundle, { limit = 3 } = {}) {
  ...
  const planetSignificators = resultData(bundle?.kp_planet_significators);
  const kpPlanets = resultData(bundle?.kp_planets);
  const currentDasha = resultData(bundle?.current_vdasha);

  addSignificatorBreadth(scores, planetSignificators);
  addDashaRoles(scores, currentDasha || {});
  addLordLayers(scores, kpPlanets);
  ...
  return {
    ...
    formula: {
      unique_significator_house: 1,
      dasha_weights: DASHA_WEIGHTS,
      ascendant_weights: ASCENDANT_WEIGHTS,
      moon_weights: MOON_WEIGHTS,
      selection_rule: `Top ${limit} planets by score. Ties use alphabetical order for deterministic output.`
    },
...
function doubleConfirmation(dominantResult, deterministicNumerology) {
  const dominantSet = new Set(
    (dominantResult?.dominant_planets || []).map(item => normalisePlanet(item.planet))
  );
  ...
  const checks = [
    ...
    ...item,
    matches_dominant_chart: Boolean(item.planet && dominantSet.has(item.planet))
  }));
