...
function runDirectGeometrySmoke() {
  ...
  if (!geometry.passed) throw new Error(`Direct premium geometry failed: ${JSON.stringify(geometry.issues.slice(0,12))}`);
  return { pages:svgPages.length, geometry_passed:true };
}

module.exports = { runDirectGeometrySmoke };
