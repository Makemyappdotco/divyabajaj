...
function runDirectFooterSmoke() {
  ...
  const blocking = geometry.issues.filter(issue => issue.type === 'footer_collision_risk' || issue.type === 'footer');
  if (blocking.length) throw new Error(`Direct footer safe-zone failed: ${JSON.stringify(blocking.slice(0,12))}`);
  return { footer_safe_zone_passed:true };
}

module.exports = { runDirectFooterSmoke };
